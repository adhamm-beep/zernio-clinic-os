...
export async function getStaffData(clinicId:number,branchId:number):Promise<StaffData>{
 ...
 const [members,attendance,shifts,roles]=await Promise.all([
  supabase.from("staff").select("id,staff_name,role,department,phone,email,is_active,employee_code,job_title,hire_date,employment_status,salary,notes").eq("clinic_id",clinicId).eq("branch_id",branchId).order("staff_name"),
 ...
 return {members:(members.data??[]) as StaffData["members"],attendance:(attendance.data??[]) as StaffData["attendance"],shifts:(shifts.data??[]) as StaffData["shifts"],roles:(roles.data??[]) as StaffData["roles"]};
}

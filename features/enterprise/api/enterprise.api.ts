import{createClient}from"@/lib/supabase/client";const s=createClient();export async function getEnterprise(c:number,b:number,access={tasks:false,audit:false,manage:false}){const empty=Promise.resolve({data:[],error:null});const[t,n,a,w,r,p,current]=await Promise.all([access.tasks?s.from("enterprise_tasks").select("*,assignee:staff!enterprise_tasks_assigned_to_fkey(staff_name)").eq("clinic_id",c).eq("branch_id",b).order("created_at",{ascending:false}):empty,s.from("enterprise_notifications").select("*").eq("clinic_id",c).order("created_at",{ascending:false}).limit(50),access.audit?s.from("enterprise_audit_log").select("*,actor:staff(staff_name)").eq("clinic_id",c).order("created_at",{ascending:false}).limit(100):empty,s.from("enterprise_workflows").select("*").eq("clinic_id",c).eq("branch_id",b).order("name"),access.manage?s.from("enterprise_workflow_runs").select("*").eq("clinic_id",c).order("created_at",{ascending:false}).limit(50):empty,s.from("enterprise_preferences").select("*").maybeSingle(),s.rpc("current_staff_id")]);const e=t.error||n.error||a.error||w.error||r.error||p.error||current.error;if(e)throw new Error(e.message);return{tasks:t.data??[],notifications:n.data??[],audit:a.data??[],workflows:w.data??[],runs:r.data??[],preference:p.data,currentStaffId:Number(current.data)};}

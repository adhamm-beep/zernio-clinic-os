...
export default function DashboardOverview() {
  ...
  const canManageAppointments=usePermission("appointments.manage").allowed;
  ...
  return <div className="space-y-3" dir={isArabic ? "rtl" : "ltr"}>
    {secureMetrics.error&&<div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm font-bold text-rose-700">{text("Metric results could not be loaded","تعذر تحميل نتائج الصلاحيات")}: {secureMetrics.error.message}</div>}
    <section className="overflow-hidden rounded-[28px] bg-[#071826] text-white shadow-xl">
      <div className="flex flex-col gap-3 p-4 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.22em] text-cyan-300">ZERNIO LIVE OPERATIONS</p><h1 className="mt-1 text-2xl font-black">{text("Your clinic, live and under control", "عيادتك لحظة بلحظة")}</h1><p className="mt-1 text-xs text-slate-300">{text("Appointments, completed patients and billing in one connected workspace.", "المواعيد والمرضى المكتملون والفواتير في مساحة تشغيل واحدة مترابطة.")}</p></div><div className="flex gap-2"><Link href="/appointments" className="rounded-lg bg-cyan-400 px-3 py-2 text-xs font-black text-slate-950">{text("New appointment", "موعد جديد")}</Link><button onClick={() => { void appointmentsQuery.refetch(); void paymentsQuery.refetch(); }} className="grid size-9 place-items-center rounded-lg bg-white/10" aria-label={text("Refresh", "تحديث")}><RefreshCw className={`size-4 ${busy ? "animate-spin" : ""}`}/></button></div></div>
      <div className="grid border-t border-white/10 sm:grid-cols-2 xl:grid-cols-4">{metrics.map(([label, value, Icon]) => <div key={label} className="flex items-center gap-3 border-white/10 p-3 sm:border-s"><span className="grid size-9 place-items-center rounded-xl bg-white/10"><Icon className="size-4 text-cyan-300"/></span><div><p className="text-[10px] font-bold text-slate-400">{label}</p><p className="text-xl font-black">{value}</p></div></div>)}</div>
    </section>
    ...
    <section className="grid gap-5 xl:grid-cols-[1.65fr_.75fr]"><div className="overflow-hidden rounded-3xl border bg-white shadow-sm"><div className="flex items-center justify-between border-b p-5"><div><h2 className="text-xl font-black">{text("Daily appointment flow", "سير مواعيد اليوم")}</h2><p className="text-sm text-slate-500">{appointments.length} {text("appointments match the active filters", "موعد مطابق للفلاتر")}</p></div><CalendarDays className="size-6 text-cyan-600"/></div><div className="grid gap-3 p-4">{appointments.length ? appointments.map(item => { const customer = `${item.customers?.first_name ?? ""} ${item.customers?.last_name ?? ""}`.trim() || text("Patient", "مريض"); const time = new Intl.DateTimeFormat(isArabic ? "ar-SA" : "en-US", { hour: "numeric", minute: "2-digit", hour12: true, timeZone: "Asia/Riyadh" }).format(new Date(item.appointment_at)); return <button type="button" onClick={()=>setSelectedAppointment(item)} key={item.id} className={`rounded-2xl border p-4 text-start transition hover:-translate-y-0.5 hover:shadow-md ${statusStyles[item.status]}`}><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-xl bg-white/70"><UserRound className="size-5"/></span><div><h3 className="font-black">{customer}</h3><p className="text-xs opacity-80" dir="ltr">{item.customers?.phone || "—"}</p></div></div><span className="rounded-full bg-white/80 px-3 py-1 text-xs font-black">{statusLabel(item.status)}</span></div><div className="mt-4 grid gap-2 text-sm sm:grid-cols-3"><span className="flex items-center gap-2"><Clock3 className="size-4"/>{time}</span><span>{item.staff?.staff_name || text("Department", "قسم")}</span><span>{serviceName(item.service_id)}</span></div></button>; }) : <div className="py-20 text-center text-slate-400"><CalendarDays className="mx-auto mb-3 size-10"/><p>{text("No appointments match these filters.", "لا توجد مواعيد مطابقة لهذه الفلاتر.")}</p></div>}</div></div>
    ...
    </section><AppointmentCustomerWorkspace appointment={selectedAppointment} payments={paymentsQuery.data??[]} open={!!selectedAppointment} onOpenChange={v=>{if(!v)setSelectedAppointment(null)}} canManage={canManageAppointments}/>
  </div>;
}

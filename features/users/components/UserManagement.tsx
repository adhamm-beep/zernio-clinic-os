...
export default function UserManagement() {
  ...
  const permissionGroups=Object.entries(permissionCatalog.reduce<Record<string,typeof permissionCatalog>>((groups,permission)=>{(groups[permission.module]??=[]).push(permission);return groups;},{}));
  ...
  return <div className="space-y-6">
    ...
    <Dialog open={Boolean(permissionUser)} onOpenChange={next=>{if(!next)setPermissionUser(null)}}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
        <DialogHeader><DialogTitle className="text-xl font-black">{text("User permissions","صلاحيات المستخدم")}: {permissionUser?.staff_name}</DialogTitle><DialogDescription>{text("Select every page and action this user can access. These choices override the role defaults.","حدد كل صفحة وإجراء يمكن لهذا المستخدم الوصول إليه. هذه الاختيارات تتغلب على إعدادات الدور.")}</DialogDescription></DialogHeader>
        <div className="space-y-4 py-2">{permissionGroups.map(([module,permissions])=><section key={module} className="rounded-2xl border"><div className="flex items-center justify-between border-b bg-slate-50 px-4 py-3"><h3 className="font-black text-slate-800">{module}</h3><button type="button" className="text-xs font-bold text-[#425f70]" onClick={()=>setPermissionValues(current=>{const next=new Set(current);const all=permissions.every(permission=>next.has(permission.id));permissions.forEach(permission=>all?next.delete(permission.id):next.add(permission.id));return next})}>{permissions.every(permission=>permissionValues.has(permission.id))?text("Clear all","إلغاء الكل"):text("Select all","تحديد الكل")}</button></div><div className="grid gap-2 p-3 sm:grid-cols-2">{permissions.map(permission=><label key={permission.id} className="flex cursor-pointer items-center gap-3 rounded-xl p-2.5 hover:bg-slate-50"><input type="checkbox" checked={permissionValues.has(permission.id)} onChange={event=>setPermissionValues(current=>{const next=new Set(current);if(event.target.checked)next.add(permission.id);else next.delete(permission.id);return next})} className="size-4 accent-[#425f70]"/><span><span className="block text-sm font-bold">{permission.name}</span><span className="block text-[11px] text-slate-400">{permission.code}</span></span></label>)}</div></section>)}</div>

...
import AppointmentStatusFilterSelect from "@/features/appointments/components/AppointmentStatusFilterSelect";
...
export default function CalendarFilters({ events, filters, onChange }: Props) {
  ...
  return <div className="grid gap-2 rounded border bg-white p-3 shadow-sm md:grid-cols-3">
    ...
    <div><label className="mb-1 block text-sm font-medium text-gray-700">{text("Status", "الحالة")}</label><AppointmentStatusFilterSelect value={filters.status} onChange={(status) => onChange({ ...filters, status })} /></div>
  </div>;
}

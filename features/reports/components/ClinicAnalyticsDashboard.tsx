  ...
  return <div className="space-y-7">
    ...
    <section id="marketing" className="scroll-mt-24 space-y-5">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Metric label="Cost per booking" value={data.marketing.costPerBooking == null ? "Not available" : money(data.marketing.costPerBooking)} hint="Add campaign spend to calculate" />
      </div>
      <div className="rounded-2xl border bg-white p-6 shadow-sm"><div className="flex items-center gap-3"><Megaphone className="text-fuchsia-600" /><div><h2 className="font-bold">Lead sources & campaign attribution</h2><p className="text-sm text-slate-500">Bookings, conversion, and collected revenue linked to appointment source</p></div></div>{data.marketing.sources.length === 0 ? <p className="py-10 text-center text-sm text-slate-500">No lead sources are recorded yet.</p> : <div className="mt-5 overflow-x-auto"><table className="w-full min-w-[600px] text-left text-sm"><thead><tr className="border-b text-slate-500"><th className="pb-3 font-medium">Source</th><th className="pb-3 font-medium">Bookings</th><th className="pb-3 font-medium">Converted</th><th className="pb-3 font-medium">Conversion</th><th className="pb-3 font-medium">Revenue</th></tr></thead><tbody>{data.marketing.sources.map((item) => <tr key={item.source} className="border-b last:border-0"><td className="py-4 font-semibold capitalize">{item.source}</td><td className="py-4">{item.leads}</td><td className="py-4">{item.converted}</td><td className="py-4"><span className="inline-flex items-center gap-1"><Target className="h-4 w-4 text-emerald-600" />{item.conversionRate}%</span></td><td className="py-4 font-semibold">{money(item.revenue)}</td></tr>)}</tbody></table></div>}</div>
    </section>

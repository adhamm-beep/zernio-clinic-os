    ...
    <div className="flex h-[60px] items-center gap-2">
      ...
      <div className="relative">
        ...
        {notificationsOpen&&<div className="absolute end-0 top-12 z-50 w-[min(92vw,390px)] overflow-hidden rounded-2xl border bg-white shadow-2xl">
          ...
          <div className="max-h-[420px] overflow-y-auto">{items.length===0?<p className="p-8 text-center text-sm text-slate-500">{text("No notifications yet.","لا توجد إشعارات حتى الآن.")}</p>:items.map(item=><button type="button" key={item.id} onClick={()=>void openNotification(item)} className={`block w-full border-b p-4 text-start transition hover:bg-slate-50 ${item.is_read?"bg-white":"bg-emerald-50/70"}`}><span className="flex items-start gap-3">{!item.is_read&&<i className="mt-1.5 size-2 shrink-0 rounded-full bg-emerald-500"/>}<span><strong className="block text-sm text-slate-950">{localizedNotification(item.title,isArabic)}</strong><span className="mt-1 block text-xs leading-5 text-slate-600">{localizedNotification(item.message,isArabic)}</span><time className="mt-2 block text-[10px] text-slate-400">{new Intl.DateTimeFormat(isArabic?"ar-SA-u-nu-latn":"en-SA",{dateStyle:"medium",timeStyle:"short",timeZone:"Asia/Riyadh"}).format(new Date(item.created_at))}</time></span></span></button>)}</div>
        </div>}
      </div>

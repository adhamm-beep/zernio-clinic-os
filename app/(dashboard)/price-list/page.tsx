...
type ServiceForm = { id?: number; name: string; nameEn: string; nameAr: string; code: string; category: string; duration: string; price: string; startingFrom: boolean; deviceIds: number[] };
type VariantForm = { id?: number; serviceId: number; name: string; nameEn: string; nameAr: string; price: string; startingFrom: boolean };
const emptyService: ServiceForm = { name: "", nameEn: "", nameAr: "", code: "", category: "", duration: "30", price: "0", startingFrom: false, deviceIds: [] };
...
export default function PriceListPage() {
  ...
  const serviceLabel = (service: MasterService) => isArabic ? service.name_ar ?? service.name : service.name_en ?? service.name;
  const variantLabel = (variant: MasterServiceVariant) => isArabic ? variant.name_ar ?? variant.name : variant.name_en ?? variant.name;
  ...
  const serviceMutation = useMutation({
    ...
    onSuccess: async () => { await refresh(); setServiceForm(null); toast.success(text("Service saved and connected across Zernio", "تم حفظ الخدمة وربطها في جميع أجزاء زيرنيو")); },
    onError: (failure: Error) => toast.error(failure.message),
  });
  const variantMutation = useMutation({
    mutationFn: (input: SaveVariantInput) => saveVariant(input),
    onSuccess: async () => { await refresh(); setVariantForm(null); toast.success(text("Material and price saved successfully", "تم حفظ المادة والسعر بنجاح")); },
    onError: (failure: Error) => toast.error(failure.message),
  });
  const removeServiceMutation = useMutation({
    mutationFn: ({ serviceId }: { serviceId: number }) => unlinkServiceFromProvider(serviceId, selectedProviderId),
    onSuccess: async () => { await refresh(); toast.success(text("Service removed from this provider. Previous records are protected.", "تمت إزالة الخدمة من مقدم الخدمة مع حماية السجلات السابقة.")); },
    onError: (failure: Error) => toast.error(failure.message),
  });
  const removeVariantMutation = useMutation({
    ...
    onSuccess: async () => { await refresh(); toast.success(text("Material removed from this provider. Previous invoices are protected.", "تمت إزالة المادة من مقدم الخدمة مع حماية الفواتير السابقة.")); },
    onError: (failure: Error) => toast.error(failure.message),
  });
  ...
  function submitService(event: FormEvent<HTMLFormElement>) {
    ...
    serviceMutation.mutate({
      ...
      name: serviceForm.nameEn, nameEn: serviceForm.nameEn, nameAr: serviceForm.nameAr, code: serviceForm.code, category: serviceForm.category || selectedDepartment?.category || serviceForm.nameEn,
  ...
  function submitVariant(event: FormEvent<HTMLFormElement>) {
    ...
    variantMutation.mutate({
      ...
      serviceId: variantForm.serviceId, name: variantForm.nameEn, nameEn: variantForm.nameEn, nameAr: variantForm.nameAr, price: Number(variantForm.price), startingFrom: variantForm.startingFrom,
    });
  }
  function editService(service: MasterService) {
    ...
    setServiceForm({ id: service.id, name: service.name, nameEn: service.name_en ?? service.name, nameAr: service.name_ar ?? service.name, code: service.code ?? "", category: service.category ?? "", duration: String(service.duration_minutes), price: String(departmentPrice?.price ?? service.default_price), startingFrom: departmentPrice?.is_starting_from ?? service.price_starting_from, deviceIds: data?.serviceDevices.filter((link) => link.service_id === service.id).map((link) => link.device_id) ?? [] });
  }

  if (isLoading) return <div className="rounded-3xl border bg-white p-12 text-center shadow-sm">{text("Loading the service catalog…","جاري تحميل دليل الخدمات…")}</div>;
  if (error || !data) return <div className="rounded-3xl bg-red-50 p-8 text-red-700">{error?.message ?? text("Unable to load the catalog.", "تعذر تحميل دليل الخدمات.")}</div>;
    ...
    <div className="space-y-7">
      <section className="overflow-hidden rounded-3xl bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 p-7 text-white shadow-xl">
        ...
        <div className="mt-7 grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-2xl bg-white/10 p-4 backdrop-blur"><Boxes className="h-5 w-5 text-cyan-300" /><p className="mt-3 text-2xl font-bold">{totalMaterials}</p><p className="text-sm text-slate-300">{text("Materials & options", "المواد والخيارات")}</p></div>
          <div className="rounded-2xl bg-white/10 p-4 backdrop-blur"><CircleDollarSign className="h-5 w-5 text-amber-300" /><p className="mt-3 text-2xl font-bold">{text("SAR", "ر.س")}</p><p className="text-sm text-slate-300">{text("Live invoice prices", "أسعار الفواتير المعتمدة")}</p></div>
        </div>
      </section>

      <section className="rounded-3xl border bg-white p-5 shadow-sm">
        <p className="mb-3 text-sm font-semibold text-slate-500">{text("Choose doctor or department", "اختر الطبيب أو القسم")}</p>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div><h2 className="text-2xl font-black text-slate-900">{providerName}</h2><p className="text-sm text-slate-500">{text("Services and exact prices currently available across the system", "الخدمات والأسعار الدقيقة المعتمدة في جميع أجزاء النظام")}</p></div>
        <div className="relative w-full sm:w-80"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={text("Search service or material...", "ابحث عن خدمة أو مادة...")} className="pl-9" /></div>
      </div>
      ...
      {!services.length && <div className="rounded-3xl border border-dashed bg-white p-14 text-center"><Boxes className="mx-auto h-10 w-10 text-slate-300" /><p className="mt-4 font-bold text-slate-800">{text("No matching services", "لا توجد خدمات مطابقة")}</p><p className="mt-1 text-sm text-slate-500">{text(`Add the first service for ${providerName}.`, `أضف أول خدمة لـ ${providerName}.`)}</p></div>}

      <Dialog open={serviceForm !== null} onOpenChange={(open) => !open && setServiceForm(null)}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle>{serviceForm?.id ? text("Edit service", "تعديل الخدمة") : text("Add service", "إضافة خدمة")} — {providerName}</DialogTitle></DialogHeader>{serviceForm && <form onSubmit={submitService} className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium">{text("English name", "الاسم الإنجليزي")}<Input required value={serviceForm.nameEn} onChange={(event) => setServiceForm({ ...serviceForm, name: event.target.value, nameEn: event.target.value })} className="mt-1" dir="ltr" /></label><label className="text-sm font-medium">{text("Arabic name", "الاسم العربي")}<Input required value={serviceForm.nameAr} onChange={(event) => setServiceForm({ ...serviceForm, nameAr: event.target.value })} className="mt-1" dir="rtl" /></label></div><div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium">{text("Code", "الرمز")}<Input required value={serviceForm.code} onChange={(event) => setServiceForm({ ...serviceForm, code: event.target.value })} className="mt-1" placeholder="BOTOX" /></label><label className="text-sm font-medium">{text("Category", "التصنيف")}<Input required value={serviceForm.category} disabled={selectedProviderId < 0} onChange={(event) => setServiceForm({ ...serviceForm, category: event.target.value })} className="mt-1" /></label></div><div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium">{text("Duration (minutes)", "المدة (بالدقائق)")}<Input required type="number" min="5" step="5" value={serviceForm.duration} onChange={(event) => setServiceForm({ ...serviceForm, duration: event.target.value })} className="mt-1" /></label>{selectedProviderId < 0 && <label className="text-sm font-medium">{text("Service price", "سعر الخدمة")}<Input required type="number" min="0" step="0.01" value={serviceForm.price} onChange={(event) => setServiceForm({ ...serviceForm, price: event.target.value })} className="mt-1" /></label>}</div>{selectedProviderId < 0 && <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={serviceForm.startingFrom} onChange={(event) => setServiceForm({ ...serviceForm, startingFrom: event.target.checked })} />{text("Price starts from this amount", "السعر يبدأ من هذا المبلغ")}</label>}<fieldset className="rounded-xl border p-3"><legend className="px-1 text-sm font-semibold">{text("Required device (optional)", "الجهاز المطلوب (اختياري)")}</legend><div className="grid gap-2 sm:grid-cols-2">{data.devices.map((device) => <label key={device.id} className="flex items-center gap-2 rounded-lg bg-slate-50 p-2 text-sm"><input type="checkbox" checked={serviceForm.deviceIds.includes(device.id)} onChange={(event) => setServiceForm({ ...serviceForm, deviceIds: event.target.checked ? [...serviceForm.deviceIds, device.id] : serviceForm.deviceIds.filter((id) => id !== device.id) })} />{device.name} — {data.rooms.find((room) => room.id === device.room_id)?.name ?? text("No room", "بدون غرفة")}</label>)}</div></fieldset><Button type="submit" className="w-full" disabled={serviceMutation.isPending}>{serviceMutation.isPending ? text("Saving...", "جارٍ الحفظ...") : text("Save and connect everywhere", "حفظ وربط في كل النظام")}</Button></form>}</DialogContent></Dialog>

      <Dialog open={variantForm !== null} onOpenChange={(open) => !open && setVariantForm(null)}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle>{variantForm?.id ? text("Edit material", "تعديل المادة") : text("Add material", "إضافة مادة")}</DialogTitle></DialogHeader>{variantForm && <form onSubmit={submitVariant} className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium">{text("English name", "الاسم الإنجليزي")}<Input required value={variantForm.nameEn} onChange={(event) => setVariantForm({ ...variantForm, name: event.target.value, nameEn: event.target.value })} className="mt-1" dir="ltr" /></label><label className="text-sm font-medium">{text("Arabic name", "الاسم العربي")}<Input required value={variantForm.nameAr} onChange={(event) => setVariantForm({ ...variantForm, nameAr: event.target.value })} className="mt-1" dir="rtl" /></label></div><label className="block text-sm font-medium">{text(`Price for ${providerName}`, `سعر ${providerName}`)}<Input required type="number" min="0" step="0.01" value={variantForm.price} onChange={(event) => setVariantForm({ ...variantForm, price: event.target.value })} className="mt-1" /></label><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={variantForm.startingFrom} onChange={(event) => setVariantForm({ ...variantForm, startingFrom: event.target.checked })} />{text("Price starts from this amount", "السعر يبدأ من هذا المبلغ")}</label><div className="rounded-xl bg-amber-50 p-3 text-xs text-amber-800">{text(`This price belongs to ${providerName} only and will be used automatically in treatment sessions and invoices.`, `هذا السعر خاص بـ ${providerName} فقط وسيُستخدم تلقائيًا في الإجراءات والفواتير.`)}</div><Button type="submit" className="w-full" disabled={variantMutation.isPending}>{variantMutation.isPending ? text("Saving...", "جارٍ الحفظ...") : text("Save material and price", "حفظ المادة والسعر")}</Button></form>}</DialogContent></Dialog>
    </div>
  );
}

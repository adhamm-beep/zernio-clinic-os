...
export default function CustomersPage() {
  ...
  const [search, setSearch] = useState(""); const [doctor, setDoctor] = useState("all"); const [branch, setBranch] = useState("all"); const [tag, setTag] = useState("all"); const [referral, setReferral] = useState("all"); const [page, setPage] = useState(1);
  ...
  return <div className="space-y-3" dir="rtl"><header className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="flex items-center gap-2 text-2xl font-black"><UsersRound className="text-sky-600" />المرضى <span className="rounded-full bg-sky-100 px-3 py-1 text-sm text-sky-700">{filtered.length}</span></h1><p className="text-sm text-slate-500">سجل موحد للبيانات والمواعيد والرصيد والإحالات والعلامات</p></div>{clinic && selectedBranch && <div className="flex gap-2">{access.can("customers.create", "customers.manage") && <AddCustomerDialog clinicId={clinic.id} branchId={selectedBranch.id} />}{access.can("appointments.create", "appointments.manage") && <AddAppointmentDialogV2 clinicId={clinic.id} branchId={selectedBranch.id} />}</div>}</header>
    <section className="grid gap-2 rounded-xl border bg-white p-3 shadow-sm md:grid-cols-3 xl:grid-cols-6"><label className="flex items-center gap-2 rounded-lg border px-3"><Filter className="size-4 text-slate-400" /><input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="الاسم، الهاتف، الملف، الهوية..." className="h-10 min-w-0 flex-1 outline-none" /></label><Select value={branch} onChange={filter(setBranch)} label="كل الفروع" options={branches} /><Select value={doctor} onChange={filter(setDoctor)} label="طبيب" options={doctors} /><Select value={tag} onChange={filter(setTag)} label="كل العلامات" options={tags} /><Select value={referral} onChange={filter(setReferral)} label="كل الإحالات" options={referrals} /><button onClick={() => { setSearch(""); setBranch("all"); setDoctor("all"); setTag("all"); setReferral("all"); setPage(1); }} className="h-10 rounded-lg border font-bold">إزالة الفلاتر</button></section>

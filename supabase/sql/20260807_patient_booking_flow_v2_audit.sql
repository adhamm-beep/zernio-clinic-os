with checks as (
  ...
  union all select 'booking_v2_functions',count(*)::bigint,4 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in('patient_booking_providers','patient_provider_services','patient_book_appointment','patient_select_payment_method')
